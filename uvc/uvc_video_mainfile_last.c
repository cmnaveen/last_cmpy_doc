...
int threadfn_pop_queue(void *data )
{
    mm_segment_t old_fs;
    int ret;
    fp = filp_open("/home/naveen/sample1.yuv", O_WRONLY | O_CREAT ,0664); // | O_TRUNC, 0664);
    if(fp == NULL)
    ...
    {    //The thread cannot exit until the kthread_should_stop() function returns 1
        ...
        {
            if(front[d] == NULL)
                printk("front[%d] is empty\n",d);

            else{
                int No_of_packets=0,k=0;
                loff_t current_pos = 0 ;

                No_of_packets = front[d]->dp_urb->number_of_packets;
                for(k=0 ; k < No_of_packets ; k++)

                {
                    unsigned int actual_len=0,len=0;
                    char *buf;
                    int temp_len=1024;
                    actual_len = front[d]->dp_urb->iso_frame_desc[k].actual_length;
                    current_pos = front[d]->dp_urb->iso_frame_desc[k].offset;
                    memcpy(buf,(char *)front[d]->dp_urb->transfer_buffer,actual_len);

                    if((actual_len != 0) &&(actual_len != 12))
                    {
                        if(fflag == 1){
                            current_pos = current_pos+12;
                            actual_len = actual_len-12;
                        }

                        if(actual_len <= temp_len){
                            len = vfs_write(fp, buf+current_pos,actual_len, &fp->f_pos);
                            if (len != actual_len) {
                            ...
                            while(actual_len != 0) {
                                len = vfs_write(fp,buf+current_pos,temp_len, &fp->f_pos);
                                if (len != actual_len) {
                                    printk(KERN_ERR "vfs_write failed\n");
                                }
                ...
                if ((ret = usb_submit_urb(front[d]->dp_urb, GFP_ATOMIC)) < 0) {
...
{   //write it into a file
    ...
    fp = filp_open("/home/naveen/sample.yuv", O_WRONLY | O_CREAT | O_TRUNC, 0664);
    if(fp == NULL)
        printk(KERN_ALERT "filp_open error!!.\n");
            ...
            while(front->next != NULL)
            {
                ...
                for(k=0 ; k < No_of_packets ; k++)
                {
                    ...
                    if((actual_len != 0) &&(actual_len != 12))
                    {
                        ...
                        if(actual_len <= temp_len){
                            len = vfs_write(fp, (char *)(tail->dp_urb->transfer_buffer)+current_pos,actual_len, &fp->f_pos);
                            if (len < 0) {
                                printk(KERN_ERR "vfs_write failed\n");
                            }
                            current_pos = current_pos + len;
                            ...
                            while(actual_len != 0) {
                                len = vfs_write(fp,(char*)(tail->dp_urb->transfer_buffer)+current_pos,temp_len, &fp->f_pos);
                                if (len < 0) {
                                    printk(KERN_ERR "vfs_write failed\n");
                                }
                                actual_len = actual_len - len;
