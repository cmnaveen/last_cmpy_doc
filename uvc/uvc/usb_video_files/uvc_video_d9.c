...
/*spin lock*/
static spinlock_t my_irqlock;// = SPIN_LOCK_UNLOCKED;


/*copy video data in a file*/  
static struct queue
{
	struct urb *dp_urb;
	struct queue *next;
}*front ;                                                                                 
/*------- copying the data into a file -----------*/


static int n=0;
void Enqueue(struct urb *urb)
{
	struct queue *newNode;
	unsigned long Enq_flag;
	newNode = kzalloc(sizeof(struct queue),GFP_KERNEL);
	...
	if(front == NULL){
		spin_lock_irqsave(&my_irqlock, Enq_flag);
		front = newNode;
		spin_unlock_irqrestore(&my_irqlock, Enq_flag);

		for (n = 0; n < front->dp_urb->number_of_packets; ++n) {
			if((front->dp_urb->iso_frame_desc[n].actual_length) !=0){
				printk("%s:%d:if--->front actual_length:%d\n",__func__,__LINE__
						,front->dp_urb->iso_frame_desc[n].actual_length);
			}
		}
	}
	else
	{
		...
		temp->next = newNode;
		spin_unlock_irqrestore(&my_irqlock, Enq_flag);
		for (n = 0; n < temp->dp_urb->number_of_packets; ++n) {
			if((temp->dp_urb->iso_frame_desc[n].actual_length) !=0){
				printk("%s:%d:else--->temp actual_length:%d\n",__func__,__LINE__
						,temp->dp_urb->iso_frame_desc[n].actual_length);
			}
		}
	}
...
int threadfn_pop_queue(void *data )
{   //write it into a file
	struct file *fp;
	loff_t pos = 0 ;
	unsigned long Dq_flag;
	int ret;
	fp = filp_open("/home/naveen/sample.yuv", O_WRONLY | O_CREAT | O_APPEND, 0664);
	if(fp == NULL)
		printk(KERN_ALERT "filp_open error!!.\n");

	printk("%s: called from this LINE:%d\n", __func__,__LINE__);

	while (!kthread_should_stop()) 
	{ 	//The thread cannot exit until the kthread_should_stop() function returns 1
		...
		else
		{
			while(front != NULL){
				struct queue *tail; 
				int No_of_packets=0,k=0;
				loff_t current_pos = 0 ;
				int endof=0;
				//			while(front->next != NULL)
				//				front=front->next;
				spin_lock_irqsave(&my_irqlock, Dq_flag); // it will disable the interrupts
				tail = front;
				spin_unlock_irqrestore(&my_irqlock, Dq_flag);

				No_of_packets = tail->dp_urb->number_of_packets;       
				printk("LINE:%d :No_of_packets:%d\n",__LINE__,tail->dp_urb->number_of_packets);
				for(k=0 ; k < No_of_packets ; k++)
				{
					unsigned long int actual_len=0,len=0;
					int o=0,m=0;
					actual_len = tail->dp_urb->iso_frame_desc[k].actual_length;
					current_pos = tail->dp_urb->iso_frame_desc[k].offset;
					endof = tail->dp_urb->iso_frame_desc[2].offset;
					if(actual_len !=0){
					for(m = current_pos ; m < (current_pos+endof) ; m++)
					{ 
						printk("%s: called from this LINE:%d\n", __func__,__LINE__);
						current_pos++;
						if((((char *)(tail->dp_urb->transfer_buffer)+m) == (char*)0xFF) && 
									((char *)(tail->dp_urb->transfer_buffer)+(m+1)) == (char *)0xD8)
							printk("%s: called from this LINE:%d\n", __func__,__LINE__);
							for(o = m ; o < (current_pos+endof) ; o++){
								len = vfs_write(fp, (char *)(tail->dp_urb->transfer_buffer)+current_pos,1, 
																&fp->f_pos);
								current_pos++;pos++;
								if( (((char *)(tail->dp_urb->transfer_buffer)+o) == (char*)0xFF) &&
										((char *)(tail->dp_urb->transfer_buffer)+(o+1) == (char *)0xD9)) 
								{
								printk("%s: called from this LINE:%d\n", __func__,__LINE__);
									o=o+12;m=o;
				...
				if ((ret = usb_submit_urb(tail->dp_urb, GFP_ATOMIC)) < 0) {
					printk(KERN_ERR "Failed to resubmit video URB %d\n",ret);
				}
				spin_lock_irqsave(&my_irqlock, Dq_flag); // it will disable the interrupts
				front = front->next;	
				spin_unlock_irqrestore(&my_irqlock, Dq_flag);
				kfree(tail);//do_exit(0);
			}
		}
		printk("%s: called from this LINE:%d\n", __func__,__LINE__);
...
//	int ret;
	...
	/* ----- my spin lock -----*/
//	spin_lock_irqsave(&my_irqlock, flags);
	Enqueue(urb);
//	spin_unlock_irqrestore(&my_irqlock, flags);
...
/*-----thread------*/ 
	...
	mythread = kthread_run(threadfn_pop_queue,NULL,"kernel_thread");
	if (mythread)
