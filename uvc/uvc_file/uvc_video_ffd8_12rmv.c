        ...
        else
        {
            while(front != NULL)
            {
                ...
                for(k=0 ; k < No_of_packets ; k++)
                {
                    ...
                    int temp_len=1024,endoff=0,m=0,total=0;
                    actual_len = tail->dp_urb->iso_frame_desc[k].actual_length;
                    current_pos = tail->dp_urb->iso_frame_desc[k].offset;
                    endoff = tail->dp_urb->iso_frame_desc[2].offset;
                    total = current_pos+endoff;
                    ...
                    if((actual_len != 0) &&(actual_len != 12))
                    {
                        if(actual_len <= temp_len){
                            ...
                            if(fflag == 1){
                                printk("%s: called from this LINE:%d\n", __func__,__LINE__);
                                for(m = current_pos ; m < total ; m++)
                                { 
                                    current_pos++;
                                    actual_len--;
                                    if(((*((char *)(tail->dp_urb->transfer_buffer))+m) == 0xFF) &&
                                            ((*((char *)(tail->dp_urb->transfer_buffer))+(m+1)) == 0xD8))
                                    {
up:
                                        printk("%s: called from this LINE:%d\n", __func__,__LINE__);
