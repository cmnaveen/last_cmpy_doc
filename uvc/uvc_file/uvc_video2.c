...
static int i=0;
void push_queue(struct urb *urb)
{
	struct queue *newNode;
        ...
        else
        {
                ...
                while(temp->next != NULL)
                        temp = temp->next;
                temp->next = newNode;
		printk("temp addr =%p :%d\n",temp,temp->dp_urb->iso_frame_desc[i].actual_length);
        }
printk("head addr =%p\n",head);
	flag =1;
...
{   //write it into a file
	...
	struct file *fp;
	unsigned long int actual_len=0,i=0,len=0,npkt=0;
	loff_t pos = 0 ;
	...
	printk("%s :head addr =%p\n",__func__,head);
	set_current_state(TASK_INTERRUPTIBLE);//changes the state of the currently executing process from TASK_RUNNING to TASK_INTERRUPTIBLE
	...
	{ 	//The thread cannot exit until the kthread_should_stop() function returns 1
		...
		else
		{
			...
			while(head != NULL) 
			{
				...
				npkt = head->dp_urb->number_of_packets;       
				for(i=0 ; i < npkt ; i++)
				{
					int atemp=0,temp_len=1024;
					actual_len = temp->dp_urb->iso_frame_desc[i].actual_length;
					current_pos = temp->dp_urb->iso_frame_desc[i].offset;

					printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
					while((actual_len <=temp_len) || (temp_len != 0)){
						len = vfs_write(fp, temp->dp_urb->transfer_buffer+current_pos, temp_len, &fp->f_pos);
						if (len < 0) {
							printk(KERN_ERR "vfs_write failed\n");
						}
						atemp = actual_len - temp_len;
						if(atemp < temp_len)
							temp_len = atemp;
						pos = pos + len;
					}
	...
	__set_current_state(TASK_RUNNING);/*The thread sleeps while waiting for the function to return 0. */
	return 0;
...
//	int ret;
	...
	/*	stream->decode(urb, stream, buf);
		...
		urvc_printk(KERN_ERR, "Failed to resubmit video URB (%d).\n",
		ret);
		}*/
...
//printk("actual length =%d\n",front->dp_urb->iso_frame_desc[i].actual_length);
